...
//! VMF allocator is a custom allocator for yyjson that prevents many tiny allocations
class VMFAllocator {
public:
	explicit VMFAllocator(Allocator &allocator)
...
//! VMFKey / vmf_key_map_t speeds up mapping from VMF key to column ID
struct VMFKey {
	const char *ptr;
	size_t len;
};
...
struct VMFKeyHash {
	inline std::size_t operator()(const VMFKey &k) const {
		size_t result;
...
struct VMFKeyEquality {
	inline bool operator()(const VMFKey &a, const VMFKey &b) const {
		if (a.len != b.len) {
			return false;
		}
...
template <typename T>
using vmf_key_map_t = unordered_map<VMFKey, T, VMFKeyHash, VMFKeyEquality>;
using vmf_key_set_t = unordered_set<VMFKey, VMFKeyHash, VMFKeyEquality>;

//! Common VMF functionality for most VMF functions
struct VMFCommon {
public:
	//! Read/Write flags
...
public:
	...
	static inline const char *ValTypeToString(yyjson_val *val) {
		...
		case YYJSON_TYPE_NULL | YYJSON_SUBTYPE_NONE:
			return VMFCommon::TYPE_STRING_NULL;
		case YYJSON_TYPE_STR | YYJSON_SUBTYPE_NOESC:
		case YYJSON_TYPE_STR | YYJSON_SUBTYPE_NONE:
			return VMFCommon::TYPE_STRING_VARCHAR;
		case YYJSON_TYPE_ARR | YYJSON_SUBTYPE_NONE:
			return VMFCommon::TYPE_STRING_ARRAY;
		case YYJSON_TYPE_OBJ | YYJSON_SUBTYPE_NONE:
			return VMFCommon::TYPE_STRING_OBJECT;
		case YYJSON_TYPE_BOOL | YYJSON_SUBTYPE_TRUE:
		case YYJSON_TYPE_BOOL | YYJSON_SUBTYPE_FALSE:
			return VMFCommon::TYPE_STRING_BOOLEAN;
		case YYJSON_TYPE_NUM | YYJSON_SUBTYPE_UINT:
			return VMFCommon::TYPE_STRING_UBIGINT;
		case YYJSON_TYPE_NUM | YYJSON_SUBTYPE_SINT:
			return VMFCommon::TYPE_STRING_BIGINT;
		case YYJSON_TYPE_NUM | YYJSON_SUBTYPE_REAL:
			return VMFCommon::TYPE_STRING_DOUBLE;
		default:
...
public:
	...
	static string FormatParseError(const char *data, idx_t length, yyjson_read_err &error, const string &extra = "") {
		...
		// Truncate, so we don't print megabytes worth of VMF
		...
		return StringUtil::Format("Malformed VMF at byte %lld of input: %s. %s Input: %s", error.pos, error.msg, extra,
		                          input);
	}
...
public:
	...
	// VMF pointer / path
	//===--------------------------------------------------------------------===//
	enum class VMFPathType : uint8_t {
		//! Extract a single value
		REGULAR = 0,
		//! Extract multiple values (when we have a '*' wildcard in the VMF Path)
		WILDCARD = 1,
	};

	//! Get VMF value using VMF path query (safe, checks the path query)
	static inline yyjson_val *Get(yyjson_val *val, const string_t &path_str, bool integral_argument) {
		...
		case '$': {
			if (ValidatePath(ptr, len, false) == VMFPathType::WILDCARD) {
				throw InvalidInputException(
				    "VMF path cannot contain wildcards if the path is not a constant parameter");
			}
			return GetUnsafe(val, ptr, len);
		}
		default: {
	...
	//! Get VMF value using VMF path query (unsafe)
	static inline yyjson_val *GetUnsafe(yyjson_val *val, const char *ptr, const idx_t &len) {
		...
		default:
			throw InternalException("VMF pointer/path does not start with '/' or '$'");
		}
	}
	...
	//! Validate VMF Path ($.field[index]... syntax), returns true if there are wildcards in the path
	static VMFPathType ValidatePath(const char *ptr, const idx_t &len, const bool binder);

private:
	//! Get VMF pointer (/field/index/... syntax)
	...
	//! Get VMF path ($.field[index]... syntax)
	static yyjson_val *GetPath(yyjson_val *val, const char *ptr, const idx_t &len);
};

template <>
inline char *VMFCommon::WriteVal(yyjson_val *val, yyjson_alc *alc, idx_t &len) {
	return yyjson_val_write_opts(val, VMFCommon::WRITE_FLAG, alc, reinterpret_cast<size_t *>(&len), nullptr);
}
template <>
inline char *VMFCommon::WriteVal(yyjson_mut_val *val, yyjson_alc *alc, idx_t &len) {
	return yyjson_mut_val_write_opts(val, VMFCommon::WRITE_FLAG, alc, reinterpret_cast<size_t *>(&len), nullptr);
}
