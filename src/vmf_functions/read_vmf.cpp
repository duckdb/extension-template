		...
		for (auto &child_type : StructType::GetChildTypes(type)) {
			...
			if (!insert_success) {
				...
				throw NotImplementedException(
				    "Duplicate name \"%s\" in struct auto-detected in VMF, try ignore_errors=true", child_type.first);
			} else {
...
void VMFScan::AutoDetect(ClientContext &context, VMFScanData &bind_data, vector<LogicalType> &return_types,
                          vector<string> &names) {
	// Change scan type during detection
	bind_data.type = VMFScanType::SAMPLE;
	...
	for (idx_t file_idx = 0; file_idx < bind_data.files.size(); file_idx++) {
		...
		VMFScanGlobalState gstate(context, bind_data);
		VMFScanLocalState lstate(context, gstate);
		if (file_idx == 0) {
			gstate.vmf_readers.emplace_back(bind_data.initial_reader.get());
		} else {
			gstate.vmf_readers.emplace_back(bind_data.union_readers[file_idx - 1].get());
		}
		...
		while (remaining != 0) {
			...
			for (idx_t i = 0; i < next; i++) {
				...
				if (val) {
					VMFStructure::ExtractStructure(val, node, true);
				}
			}
	...
	auto type = VMFStructure::StructureToType(context, node, bind_data.max_depth, bind_data.field_appearance_threshold,
	                                           bind_data.map_inference_threshold);

	// Auto-detect record type
	if (bind_data.options.record_type == VMFRecordType::AUTO_DETECT) {
		if (type.id() == LogicalTypeId::STRUCT) {
			bind_data.options.record_type = VMFRecordType::RECORDS;
		} else {
			bind_data.options.record_type = VMFRecordType::VALUES;
		}
	}
	...
	// Auto-detect columns
	if (bind_data.options.record_type == VMFRecordType::RECORDS) {
		if (type.id() == LogicalTypeId::STRUCT) {
			...
			throw BinderException("vmf_read expected records, but got non-record VMF instead."
			                      "\n Try setting records='auto' or records='false'.");
		}
	} else {
		D_ASSERT(bind_data.options.record_type == VMFRecordType::VALUES);
		return_types.emplace_back(RemoveDuplicateStructKeys(type, bind_data.ignore_errors));
		names.emplace_back("vmf");
	}
}

unique_ptr<FunctionData> ReadVMFBind(ClientContext &context, TableFunctionBindInput &input,
	...
	for (auto &kv : input.named_parameters) {
		...
		if (kv.second.IsNull()) {
			throw BinderException("read_vmf parameter \"%s\" cannot be NULL.", loption);
		}
		if (loption == "columns") {
			auto &child_type = kv.second.type();
			if (child_type.id() != LogicalTypeId::STRUCT) {
				throw BinderException("read_vmf \"columns\" parameter requires a struct as input.");
			}
			...
			for (idx_t i = 0; i < struct_children.size(); i++) {
				...
				if (val.type().id() != LogicalTypeId::VARCHAR) {
					throw BinderException("read_vmf \"columns\" parameter type specification must be VARCHAR.");
				}
				return_types.emplace_back(TransformStringToLogicalType(StringValue::Get(val), context));
			}
			D_ASSERT(names.size() == return_types.size());
			if (names.empty()) {
				throw BinderException("read_vmf \"columns\" parameter needs at least one column.");
			}
			...
			if (arg == -1) {
				...
				throw BinderException("read_vmf \"sample_size\" parameter must be positive, or -1 to sample all input "
			...
			if (arg < 0 || arg > 1) {
				throw BinderException(
				    "read_vmf_auto \"field_appearance_threshold\" parameter must be between 0 and 1");
			}
			...
			if (arg == -1) {
				...
				throw BinderException("read_vmf_auto \"map_inference_threshold\" parameter must be 0 or positive, "
			...
			if (!error.empty()) {
				throw BinderException("read_vmf could not parse \"dateformat\": '%s'.", error.c_str());
			}
			...
			auto error = StrTimeFormat::ParseFormatSpecifier(format_string, format);
			if (!error.empty()) {
				throw BinderException("read_vmf could not parse \"timestampformat\": '%s'.", error.c_str());
			}
		} else if (loption == "records") {
			auto arg = StringValue::Get(kv.second);
			if (arg == "auto") {
				bind_data->options.record_type = VMFRecordType::AUTO_DETECT;
			} else if (arg == "true") {
				bind_data->options.record_type = VMFRecordType::RECORDS;
			} else if (arg == "false") {
				bind_data->options.record_type = VMFRecordType::VALUES;
			} else {
				throw BinderException("read_vmf requires \"records\" to be one of ['auto', 'true', 'false'].");
			}
			...
			if (arg == -1) {
				...
				throw BinderException("read_vmf \"maximum_sample_files\" parameter must be positive, or -1 to remove "
	...
	if (bind_data->options.record_type == VMFRecordType::AUTO_DETECT && return_types.size() > 1) {
	...
	if (!bind_data->auto_detect) {
		...
		if (return_types.empty()) {
			throw BinderException("When auto_detect=false, read_vmf requires columns to be specified through the "
			                      "\"columns\" parameter.");
		}
		// If we are reading VALUES, we can only have one column
		if (bind_data->options.record_type == VMFRecordType::VALUES && return_types.size() != 1) {
			throw BinderException("read_vmf requires a single column to be specified through the \"columns\" "
	...
	if (bind_data->auto_detect || bind_data->options.record_type == VMFRecordType::AUTO_DETECT) {
		VMFScan::AutoDetect(context, *bind_data, return_types, names);
		bind_data->names = names;
	...
	if (bind_data->auto_detect) {
		// VMF may contain columns such as "id" and "Id", which are duplicates for us due to case-insensitivity
		// We rename them so we can parse the file anyway. Note that we can't change bind_data->names,
		// because the VMF reader gets columns by exact name, not position
...
static void ReadVMFFunction(ClientContext &context, TableFunctionInput &data_p, DataChunk &output) {
	...
	if (!gstate.names.empty()) {
		...
		D_ASSERT(gstate.bind_data.options.record_type != VMFRecordType::AUTO_DETECT);
		bool success;
		if (gstate.bind_data.options.record_type == VMFRecordType::RECORDS) {
			success = VMFTransform::TransformObject(values, lstate.GetAllocator(), count, gstate.names, result_vectors,
			                                         lstate.transform_options);
		} else {
			D_ASSERT(gstate.bind_data.options.record_type == VMFRecordType::VALUES);
			success = VMFTransform::Transform(values, lstate.GetAllocator(), *result_vectors[0], count,
			                                   lstate.transform_options);
		}
...
TableFunction VMFFunctions::GetReadVMFTableFunction(shared_ptr<VMFScanInfo> function_info) {
	TableFunction table_function({LogicalType::VARCHAR}, ReadVMFFunction, ReadVMFBind,
	                             VMFGlobalTableFunctionState::Init, VMFLocalTableFunctionState::Init);
	table_function.name = "read_vmf";
...
TableFunctionSet CreateVMFFunctionInfo(string name, shared_ptr<VMFScanInfo> info) {
	auto table_function = VMFFunctions::GetReadVMFTableFunction(std::move(info));
...
TableFunctionSet VMFFunctions::GetReadVMFFunction() {
	auto info = make_shared_ptr<VMFScanInfo>(VMFScanType::READ_VMF, VMFFormat::AUTO_DETECT,
	                                          VMFRecordType::AUTO_DETECT, true);
	return CreateVMFFunctionInfo("read_vmf", std::move(info));
}

TableFunctionSet VMFFunctions::GetReadNDVMFFunction() {
	auto info = make_shared_ptr<VMFScanInfo>(VMFScanType::READ_VMF, VMFFormat::NEWLINE_DELIMITED,
	                                          VMFRecordType::AUTO_DETECT, true);
	return CreateVMFFunctionInfo("read_ndvmf", std::move(info));
}

TableFunctionSet VMFFunctions::GetReadVMFAutoFunction() {
	auto info = make_shared_ptr<VMFScanInfo>(VMFScanType::READ_VMF, VMFFormat::AUTO_DETECT,
	                                          VMFRecordType::AUTO_DETECT, true);
	return CreateVMFFunctionInfo("read_vmf_auto", std::move(info));
}

TableFunctionSet VMFFunctions::GetReadNDVMFAutoFunction() {
	auto info = make_shared_ptr<VMFScanInfo>(VMFScanType::READ_VMF, VMFFormat::NEWLINE_DELIMITED,
	                                          VMFRecordType::AUTO_DETECT, true);
	return CreateVMFFunctionInfo("read_ndvmf_auto", std::move(info));
}
