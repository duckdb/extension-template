...
void BufferedVMFReaderOptions::Serialize(Serializer &serializer) const {
	serializer.WriteProperty<VMFFormat>(100, "format", format);
	serializer.WriteProperty<VMFRecordType>(101, "record_type", record_type);
...
BufferedVMFReaderOptions BufferedVMFReaderOptions::Deserialize(Deserializer &deserializer) {
	BufferedVMFReaderOptions result;
	deserializer.ReadProperty<VMFFormat>(100, "format", result.format);
	deserializer.ReadProperty<VMFRecordType>(101, "record_type", result.record_type);
...
void VMFScanData::Serialize(Serializer &serializer) const {
	serializer.WriteProperty<VMFScanType>(100, "vmf_type", type);
	serializer.WriteProperty<BufferedVMFReaderOptions>(101, "options", options);
	...
	serializer.WriteProperty<VMFTransformOptions>(109, "transform_options", transform_options);
...
unique_ptr<VMFScanData> VMFScanData::Deserialize(Deserializer &deserializer) {
	auto type = deserializer.ReadProperty<VMFScanType>(100, "vmf_type");
	auto options = deserializer.ReadProperty<BufferedVMFReaderOptions>(101, "options");
	...
	auto transform_options = deserializer.ReadProperty<VMFTransformOptions>(109, "transform_options");
	...
	auto result = duckdb::unique_ptr<VMFScanData>(new VMFScanData(deserializer.Get<ClientContext &>(), std::move(files), std::move(date_format), std::move(timestamp_format)));
	result->type = type;
...
void VMFTransformOptions::Serialize(Serializer &serializer) const {
...
VMFTransformOptions VMFTransformOptions::Deserialize(Deserializer &deserializer) {
	VMFTransformOptions result;
