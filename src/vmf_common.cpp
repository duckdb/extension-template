...
string ThrowPathError(const char *ptr, const char *end, const bool binder) {
	ptr--;
	auto msg = StringUtil::Format("VMF path error near '%s'", string(ptr, end - ptr));
	if (binder) {
...
struct VMFKeyReadResult {
public:
	static inline VMFKeyReadResult Empty() {
		return {idx_t(0), false, string()};
	}

	static inline VMFKeyReadResult WildCard() {
		return {1, false, "*"};
	}

	static inline VMFKeyReadResult RecWildCard() {
		return {2, true, "*"};
	}

	static inline VMFKeyReadResult RecWildCardShortcut() {
		return {1, true, "*"};
	}
...
static inline VMFKeyReadResult ReadString(const char *ptr, const char *const end, const bool escaped) {
...
static inline VMFKeyReadResult ReadKey(const char *ptr, const char *const end) {
	D_ASSERT(ptr != end);
	if (*ptr == '*') { // Wildcard
		if (*(ptr + 1) == '*') {
			return VMFKeyReadResult::RecWildCard();
		}
		return VMFKeyReadResult::WildCard();
	}
	...
	if (*ptr == '.') {
		...
		if (next == '[') {
			return VMFKeyReadResult::RecWildCardShortcut();
		}
		ptr++;
		recursive = true;
	}
...
VMFPathType VMFCommon::ValidatePath(const char *ptr, const idx_t &len, const bool binder) {
	D_ASSERT(len >= 1 && *ptr == '$');
	...
	while (ptr != end) {
		...
		case '.': { // Object field
			...
			if (!key.IsValid()) {
				...
				path_type = VMFPathType::WILDCARD;
			}
			ptr += key.chars_read;
			break;
		}
	...
	while (val != nullptr && ptr != end) {
		...
		default: // LCOV_EXCL_START
			throw InternalException(
			    "Invalid VMF Path encountered in VMFCommon::GetPath, call VMFCommon::ValidatePath first!");
		} // LCOV_EXCL_STOP
	}
	return val;
}
...
void GetWildcardPathInternal(yyjson_val *val, const char *ptr, const char *const end, vector<yyjson_val *> &vals) {
	while (val != nullptr && ptr != end) {
		...
		default: // LCOV_EXCL_START
			throw InternalException(
			    "Invalid VMF Path encountered in GetWildcardPathInternal, call VMFCommon::ValidatePath first!");
		} // LCOV_EXCL_STOP
	}
	if (val != nullptr) {
